 ...
 */
export class CaTempCreditLineGroup{
  ...
  constructor(
    ...
    public temp_sub_ca_amt_group?: number,
  ...
  static parse(json: any[]) {
    let data: CaTempCreditLineGroup[] = [];
    if (json) {
      for (let i = 0; i < json.length; i++) {
        data.push(new CaTempCreditLineGroup(
          json[i].com_code,
          json[i].ca_no,
          json[i].join_grp_code,
          json[i].sub_id,
          json[i].temp_sub_ca_amt_group,
          json[i].temp_terms,
