...
export class CaCollateralComponent implements OnInit ,OnDestroy{
  ...
  ngOnInit() {
    ...
    this.subscripData = this.creditApplicationService.eventCaHead.subscribe(
      (value : caHead ) =>
      {
          ...
          if (this.myCaHead.sbu_typ == 'P') { this.cltTypeLosn = true }
  ...
  getCltTypeDesc(value : string){
    let typeDesc = ''
    if (value =='1') { typeDesc = 'P/N'}
    else if (value =='2') { typeDesc = 'Cheque'}
    else if (value =='3') { typeDesc = 'CreditLine Guarantee'}
    else if (value =='4') { typeDesc = 'Machine'}
    else if (value =='5') { typeDesc = 'Invoice'}
    else if (value =='6') { typeDesc = 'Stock'}
    else if (value =='7') { typeDesc = 'Cash Deposit'}
    this.selectCollateral.clt_type_desc =  typeDesc ;

    if ((value == '3') || (value == '7') ) {  this.openCltDetailTab = false }
    else { this.openCltDetailTab = true ; }
  ...
  calDeposit(action : string){
    ...
    if (finAmt) {
      if ((action == 'pcn') && (this.selectCollateral.deduct_pcn) ){
        if (this.selectCollateral.deduct_pcn > 30) { this.selectCollateral.deduct_pcn = 30; }
        else if (this.selectCollateral.deduct_pcn < 0 ) { this.selectCollateral.deduct_pcn = 0 ;}
        this.selectCollateral.clt_amt = finAmt * this.selectCollateral.deduct_pcn / 100 ;
      }else
      if ((action == 'amt') && (this.selectCollateral.clt_amt) ){
