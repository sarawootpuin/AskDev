...
export class DetailComponent
  ...
  checkStep(): boolean {
    ...
    let toTerm: number = 0, fromTerm: number = 0, term = Number(this.data.terms);
    let dataStep = this.data.listStep;
    let length = dataStep.length;
    for (let i = 0; i < dataStep.length; i++) {
      if (i == 0) {
        if (dataStep[i].from_term != '1') {
          this.calIrrWarning.addMessage('- First Step Must Be 1');
          result = false;
        }
      }
      ...
      if (Number(dataStep[i].from_term) == toTerm + 1) {
        if (dataStep[i].from_term > dataStep[i].to_term) {
          this.calIrrWarning.addMessage('- Number of Terms in Table incorrect');
          result = false;
        }
      ...
      if (i == length - 1) {
        if (dataStep[i].to_term != term) {
          this.calIrrWarning.addMessage('- Invalid Period');
          result = false;
        }
      }
    }
  ...
  calculateClick() {
    ...
    if (this.data.type_cal_pricing == "1") {
      ...
      if (
        this.data.cal_inst_typ == "Fix" &&
        this.data.schedule == "R" &&
        ...
        this.data.listStep.length == 0
      ) {
        this.calIrrWarning.addMessage("- Installment");
      } else if (
        this.data.cal_inst_typ == "Float" &&
        !this.data.installment_e_vat
      ) {
        this.calIrrWarning.addMessage("- Installment");
      }
    ...
    if (calCheck) {
      ...
      this.subscription = this.appFormService
        .calculateIrr(this.data.sub_id, this.data.type_cal_pricing)
        .subscribe((data: any) => {
          //console.log(data);
          if (data.CODE == "200") {
    ...
    if (this.data.with_vat === "Y") {
      if (this.data.fin_typ === "1") {
        this.data.fin_amt_i_vat =
          this.data.asst_amt_i_vat - this.data.down_amt_i_vat;
        this.data.fin_amt_vat = this.data.asst_amt_vat - this.data.down_amt_vat;
        this.data.fin_amt_e_vat =
          this.data.fin_amt_i_vat - this.data.fin_amt_vat;
      } else if (this.data.fin_typ === "2") {
        this.data.fin_amt_i_vat =
          this.data.asst_amt_i_vat - this.data.dep_amt_i_vat;
      ...
      if (this.data.fin_typ === "1") {
        this.data.fin_amt_i_vat =
          this.data.asst_amt_e_vat - this.data.down_amt_e_vat;
      } else if (this.data.fin_typ === "2") {
        this.data.fin_amt_i_vat =
          this.data.asst_amt_e_vat - this.data.dep_amt_e_vat;
      }
      this.data.fin_amt_vat = "";
      this.data.fin_amt_e_vat = this.data.fin_amt_i_vat;
    }
    if (this.data.asst_amt_e_vat) {
      this.data.fin_ratio = Number(
        (this.data.fin_amt_e_vat ? this.data.fin_amt_e_vat : 0) /
          this.data.asst_amt_e_vat
      ).toFixed(2);
    }
  }
  ...
  calculateAsset(from: any) {
    if (from == "E") {
      if (this.data.with_vat === "Y") {
        this.data.asst_amt_vat = Number(
          ((this.data.asst_amt_e_vat ? this.data.asst_amt_e_vat : 0) *
            this.vatRate) /
            100
        ).toFixed(2);
        this.data.asst_amt_i_vat = (
          Number(this.data.asst_amt_e_vat ? this.data.asst_amt_e_vat : 0) +
          Number(this.data.asst_amt_vat)
        ).toFixed(2);
      } else {
        this.data.asst_amt_vat = 0;
        this.data.asst_amt_i_vat = this.data.asst_amt_e_vat
          ? this.data.asst_amt_e_vat
          : 0;
      }
    } else {
      if (this.data.with_vat == "Y") {
        this.data.asst_amt_e_vat = Number(
          ((this.data.asst_amt_i_vat ? this.data.asst_amt_i_vat : 0) * 100) /
            (100 + this.vatRate)
        ).toFixed(2);
        this.data.asst_amt_vat = Number(
          (this.data.asst_amt_i_vat ? this.data.asst_amt_i_vat : 0) -
            this.data.asst_amt_e_vat
        ).toFixed(2);
      } else {
        this.data.asst_amt_vat = 0;
        this.data.asst_amt_e_vat = this.data.asst_amt_i_vat
          ? this.data.asst_amt_i_vat
          : 0;
      }
    }
  ...
  calculateFin(from: any) {
    if (from == "E") {
      if (this.data.with_vat === "Y") {
        this.data.fin_amt_e_vat = this.data.fin_amt_e_vat
          ? this.data.fin_amt_e_vat
          : 0;
      ...
      if (this.data.with_vat == "Y") {
        this.data.fin_amt_i_vat = this.data.fin_amt_i_vat
          ? this.data.fin_amt_i_vat
          : 0;
        this.data.fin_amt_e_vat = Number(
          (this.data.fin_amt_i_vat * 100) / (100 + this.vatRate)
        ).toFixed(2);
        this.data.fin_amt_vat = Number(
          this.data.fin_amt_i_vat - this.data.fin_amt_e_vat
        ).toFixed(2);
    ...
    if (this.data.asst_amt_e_vat) {
      if (Number(this.data.asst_amt_e_vat)) {
        this.data.fin_ratio = Number(
          Number(this.data.fin_amt_e_vat) / Number(this.data.asst_amt_e_vat)
        ).toFixed(2);
      }
    }
  }
  ...
  calculateInstallment(have) {
    if (have == "eVat") {
      this.data.installment_e_vat = this.data.installment_e_vat
        ? this.data.installment_e_vat
        : 0;
      this.data.installment_vat = Number(
        (this.data.installment_e_vat * this.vatRate) / 100
      );
      this.data.installment_i_vat =
        Number(this.data.installment_e_vat) + Number(this.data.installment_vat);
    } else {
      this.data.installment_i_vat = this.data.installment_i_vat
        ? this.data.installment_i_vat
        : 0;
      this.data.installment_vat = Number(
        (this.data.installment_i_vat * this.vatRate) / (100 + this.vatRate)
      ).toFixed(2);
      this.data.installment_e_vat =
        Number(this.data.installment_i_vat) - Number(this.data.installment_vat);
    }
  }

  calculateLc() {
    this.data.currency = this.data.currency ? this.data.currency : 0
    this.data.duty_pcnt = this.data.duty_pcnt ? this.data.duty_pcnt : 0;
    this.data.inst_pcnt_of_asst = this.data.inst_pcnt_of_asst ? this.data.inst_pcnt_of_asst : 0
    console.log(this.data.asst_prce_forgn,this.data.curr_type,this.data.duty_pcnt);
    this.data.asst_amt_e_vat = Number(
      (Number(this.data.asst_prce_forgn) *
        Number(this.data.currency) *
        (100 + Number(this.data.duty_pcnt))) /
        100
    ).toFixed(2);
    if (
      this.data.cal_inst_typ === "Float" ||
      (this.data.schedule === "R" && this.data.cal_inst_typ === "Fix")
    ) {
      this.data.installment_e_vat = Number(
        (Number(this.data.asst_amt_e_vat) *
          Number(
            this.data.inst_pcnt_of_asst 
          )) /
          100
      ).toFixed(2);
  ...
  rateChange() {
    if (this.data.bank_code && this.data.interest_rate_type) {
      let rateSelect = this.dataBankIntRate.find(item => {
        return (
          item.id_code === this.data.bank_code &&
          item.remark === this.data.interest_rate_type
        );
      });
  ...
  addClick() {
    if (this.step.inst_e_vat) {
      this.step.inst_vat = String(
        ((Number(this.step.inst_e_vat) * this.vatRate) / 100).toFixed(2)
      );
    }
